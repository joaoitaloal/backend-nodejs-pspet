...
app.post('/api/resultados', async (req, res) => {
  try {
    const { IMAGE_URL, ERRO, ID_ALUNO, ID_PROVA, LEITURA } = req.body.data;
 
    if (ID_ALUNO == undefined || ID_PROVA == undefined || IMAGE_URL == undefined || ERRO == undefined) {
      return res.status(400).json({ error: 'Bad Request: Campos faltando' });
    }
 
    let { nota, acertos } = await avaliarLeitura( LEITURA, ID_PROVA )
...
app.put('/api/resultados/:id_aluno/:id_prova', async (req, res) => {
  try {
    ...
    const { IMAGE_URL, ERRO, LEITURA } = req.body.data;
 
    if (IMAGE_URL == undefined || ERRO == undefined) {
      return res.status(400).json({ error: 'Bad Request: Campos faltando' });
    }
